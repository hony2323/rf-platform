...
def _find_pair(folder: Path) -> tuple[Path, Path]:
    ...
    if len(data_files) > 1:
        raise ValueError(
            f"Multiple .sigmf-data files in {folder}: {[f.name for f in data_files]}"
        )
...
def reduce(folder: Path, n_samples: int, output: Path) -> None:
    ...
    if datatype not in _DTYPE_MAP:
        raise ValueError(
            f"Unsupported SigMF datatype: {datatype!r}. Supported: {list(_DTYPE_MAP)}"
        )
    ...
    if n_samples >= total_samples:
        print(
            f"Warning: file only has {total_samples:,} samples — no truncation needed."
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Reduce a SigMF recording to the first N samples."
    )
    parser.add_argument(
        "folder", type=Path, help="Folder with .sigmf-data and .sigmf-meta"
    )
    parser.add_argument(
        "--samples",
        type=int,
