"use strict";Object.defineProperty(exports, "__esModule", {value: true}); function _nullishCoalesce(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } } function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }// src/colormap.ts
...
var WaterfallRenderer = class {
  constructor(canvas, options = {}) {
    ...
    this.canvas = canvas;
    this.rowCount = _nullishCoalesce(options.rowCount, () => ( 400));
    this.bufferWidth = _nullishCoalesce(options.bufferWidth, () => ( 4096));
    this.lut = buildLut(_nullishCoalesce(options.colorMap, () => ( interpolateGrayscale)));
    this.tooltipEnabled = _nullishCoalesce(options.tooltip, () => ( false));
    this.timeBarEnabled = _nullishCoalesce(options.timeBar, () => ( false));
    this.timeBarDynamic = _nullishCoalesce(options.timeBarDynamic, () => ( false));
    this.minSpan = _nullishCoalesce(options.minSpan, () => ( 32));
    this.lazyThreshold = _nullishCoalesce(options.lazyThreshold, () => ( 4));
    this.freqFormat = _nullishCoalesce(options.freqFormat, () => ( ((hz) => hz.toFixed(1))));
    this.valueFormat = _nullishCoalesce(options.valueFormat, () => ( ((t) => (t * 100).toFixed(1) + "%")));
    this.direction = _nullishCoalesce(options.direction, () => ( "top"));
    this.isHorizontal = this.direction === "left" || this.direction === "right";
    this.flipFreq = _nullishCoalesce(options.flipFreq, () => ( false));
    this.flipFreqActual = this.isHorizontal ? !this.flipFreq : this.flipFreq;
    this.smoothPixels = _nullishCoalesce(options.smoothPixels, () => ( false));
    this.smoothZoom = _nullishCoalesce(options.smoothZoom, () => ( false));
  ...
  destroy() {
    ...
    _optionalChain([this, 'access', _ => _.tooltipEl, 'optionalAccess', _2 => _2.remove, 'call', _3 => _3()]);
    this.imgData = null;
    this.viewImg = null;
    this.ctx = null;
    this.valueBuffer = null;
  ...
  _pushRow(f) {
    ...
    if (this.timeBuffer) {
      const ts = _nullishCoalesce(_optionalChain([f, 'access', _4 => _4.header, 'access', _5 => _5[0], 'optionalAccess', _6 => _6.sent_at]), () => ( Date.now()));
  ...
  _loop() {
    ...
    if (canvas.width > 0 && canvas.height > 0 && ctx && (this.dirty || this.viewDirty)) {
      ...
      if (this.pendingPushMs >= 0) {
        ...
        _optionalChain([this, 'access', _7 => _7.onMetrics, 'optionalCall', _8 => _8(this.pendingPushMs, renderMs, isLazy)]);
        this.pendingPushMs = -1;
      }
...
exports.interpolateGrayscale = interpolateGrayscale; exports.interpolateHot = interpolateHot; exports.interpolateTurbo = interpolateTurbo; exports.buildLut = buildLut; exports.normalizeValue = normalizeValue; exports.WaterfallRenderer = WaterfallRenderer;
//# sourceMappingURL=chunk-YGG7ODPQ.cjs.map